...
#else
 // sys_printf("\n\r FLASH_ErasePage:1111\n\r"); 
  /* Wait for last operation to be completed */
  status = FLASH_WaitForLastOperation(EraseTimeout);
  //sys_printf("\n\r FLASH_ErasePage:2222\n\r");
  if(status == FLASH_COMPLETE)
  { 
    /* if the previous operation is completed, proceed to erase the page */
    FLASH->CR|= CR_PER_Set;
    FLASH->AR = Page_Address; 
    FLASH->CR|= CR_STRT_Set;
    //    sys_printf("\n\r FLASH_ErasePage:222200\n\r"); 
    /* Wait for last operation to be completed */
    status = FLASH_WaitForLastOperation(EraseTimeout);
	//   sys_printf("\n\r FLASH_ErasePage:2222111\n\r"); 
    if(status != FLASH_TIMEOUT)
    {
      /* if the erase operation is completed, disable the PER Bit */
      FLASH->CR &= CR_PER_Reset;
    }
	 //   sys_printf("\n\r FLASH_ErasePage:00003333\n\r"); 
  }
#endif /* STM32F10X_XL */
   //sys_printf("\n\r FLASH_ErasePage:ret\n\r"); 
...
{ 
  FLASH_Status status = FLASH_COMPLETE;
     // sys_printf("\n\r FLASH_WaitForLastOperation:0000\n\r"); 
  /* Check for the Flash Status */
  status = FLASH_GetBank1Status();
  /* Wait for a Flash operation to complete or a TIMEOUT to occur */
  while((status == FLASH_BUSY) && (Timeout != 0x00))
  {
   //sys_printf("\n\r FLASH_WaitForLastOperation:&&&&\n\r"); 
    status = FLASH_GetBank1Status();
    Timeout--;
  }
  if(Timeout == 0x00 )
